...
print(sol)
    ...
    Membranes are treated as rigid, immovable circles with position $\vec{r}_m$, radius $R_m$ and thickness $d$. Membranes can be permeable for certain disc types in inward direction, outward direction or both (bidirectional permeability). If  a disc with velocity $\vec{v}$ and position $\vec{r}_d$ intersects with a membrane at position $\vec{v}_m$, we get the situation displayed in \cref{fig_circle_intersections}.

    \insertfigure{images/circles_intersecting.png}{If a circle with velocity $\vec{v}$ is intersecting with a stationary circle, the circle is moving towards the stationary one if $\vec{v}\cdot(\vec{r}_d - \vec{r}_m) > 0$ because $\cos{(0^\circ \le \alpha < 90^\circ \text{ and } 270^\circ \le \alpha < 360^\circ)} > 0$}{fig_circle_intersections}
