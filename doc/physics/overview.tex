...
for sol in simplified:
    ...
    We consider two circles (2D) and want to apply the methods described in "Iterative Dynamics with Temporal Coherence" for collision handling. We'll use matrix notation: The scalar (dot) product of 2 vectors $r_1$ and $r_2$ is $r^T_1 r_2$.
    ...
    For my circles, I have no angular velocity, so $V$ is a 2n-by-1 column vector 
    ...
    For two circles this would just be 
    ...
    Next, pairwise constraints $C_k(x_i, q_i, x_j, q_j) = 0$ (with quaternions $q$ which I again don't need) are defined as abstract scalar functions. If $s$ is the number of constraints, this is an s-by-1 column vector $C$. 
    ...
    Now the paper goes on to define an algorithm to compute $\dot{C} = JV$:
