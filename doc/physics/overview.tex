...
\begin{document}
    ...
    Collision response: Bounds are stationary, no impulse is exchanged. First, solve for time $\Delta t$ when $l(t) = 0$ (see \cref{section_overlap_circles} for how it was solved):
    
    \begin{align*}
        l(t) &= |M - \vec{r}| + R_c - R_m \\
        &= \sqrt{(M_x - t\cdot v_x)^2 + (M_y - t\cdot v_y)^2} + R_c - R_m \overset{!}{=} 0\\
        &\Leftrightarrow \Delta t = \frac{ M_x v_x + M_y v_y \pm \sqrt{ (R_c - R_m)^{2}(v_x^{2} + v_y^{2}) - (M_x v_y - M_y v_x)^{2}} }{ v_x^{2} + v_y^{2} }\\
        &\Leftrightarrow \Delta t = \frac{ M\cdot\vec{v} \pm \sqrt{ (R_c - R_m)^{2}(v_x^{2} + v_y^{2}) - (M_x v_y - M_y v_x)^{2}} }{ v_x^{2} + v_y^{2} }
    \end{align*}
    
    The first point of contact happened in the past, so $\Delta t \overset{!}{<} 0$, so select the solution with $- \sqrt{...}$. Move the circle by $\Delta t$. Now, the contact normal is given by the vector 
	...
	with distance vector $\vec{r} = \vec{r_{2, 0}} - \vec{r_{1, 0}}$ and relative velocity $\vec{v} = \vec{v_2} - \vec{v_1}$ of the circles. The 2 solutions for $\Delta t$ refer to the first point of contact before overlap and the point in time exactly after the circles have passed through each other. The first contact happened in the past, so $\Delta t \overset{!}{<} 0$, so select the solution with $-\sqrt{...}$.\\
