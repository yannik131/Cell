...
for sol in simplified:
    ...
    \begin{enumerate}
        ...
        \item Solve the linear system $\Delta t (J M^{-1} J^T) \lambda = -\beta C - JV$ for $\lambda$ using Gauss-Seidel with some $\beta \in (0, \frac{1}{\Delta t}]$
        \item Calculate $V' = V + \Delta t M^{-1}J^{T}\lambda$
    \end{enumerate}
    ...
    \begin{align*}
        &\Delta t (J M^{-1} J^T) \lambda = -\beta C - JV\\
    ...
    \begin{align*}
        \lambda_k^{\mathrm{new}} = \frac{-\beta C_k - J_kV - \sum_{j<k}\Delta t(J_kM^{-1}J_j^T)\lambda_j^{\mathrm{new}} - \sum_{j>k}\Delta t(J_kM^{-1}J_j^T)\lambda_j^{\mathrm{old}}}{A_{kk}}
    \end{align*}

    Very ugly. To simplify, look at the velocity update formula: 

    \begin{align*}
        V' &= V + \Delta t M^{-1}J^T\lambda\\
        &= V + \Delta t \sum_j M^{-1}J_j^T \lambda_j.\\
        &\Leftrightarrow J_kV' = J_kV + \Delta t \sum_j (J_kM^{-1}J_j^T)\lambda_j\\
    ...
    \begin{align*}
        \Delta \lambda_k &= \frac{-\beta C_k - J_kV - \sum_{j<k}A_{kj}\lambda_j^{\mathrm{new}} - \sum_{j>k}A_{kj}\lambda_j^{\mathrm{old}}}{A_{kk}}\\
        &= \frac{-\beta C_k - J_kV - J_kV' + J_kV}{A_{kk}}\\
        &= \frac{-\beta C_k - J_kV'}{A_{kk}}
    \end{align*}

    \begin{enumerate}
        \item Calculate $\beta C_k$ depending on the constraint for collision $k$
        ...
        \item Calculate $A_{kk} = \Delta t J_kM^{-1}J_k^T$.\\
        Circle-circle: $\Delta t \begin{pmatrix} -\vec{n}^Tm_i^{-1} & \vec{n}^T m_j^{-1}\end{pmatrix}J_k^T = \Delta t(m_i^{-1} + m_j^{-1})$\\
        Bounds-circle: Entry for bounds is $0$ in $M^{-1}$, so for circle $j$ we have $\Delta t m_j^{-1}$
        \item Now calculate $\Delta \lambda_k^\mathrm{raw} = m_{\mathrm{eff},k}(-\beta C_k - J_k V')$ for collision $k$, add it to the old $\lambda$ with $\lambda_k^\mathrm{tmp} = \lambda_k^\mathrm{old} + \Delta \lambda_k^\mathrm{raw}$, set the new $\lambda$ with $\lambda_k^\mathrm{new} = \max\{0, \lambda_k^\mathrm{tmp}\}$ and calculate the actual delta with $\Delta \lambda_k = \lambda_k^\mathrm{new} - \lambda_k^\mathrm{old}$. We clamp at zero because the objects might already be moving away from each other, in which case $\lambda_k^\mathrm{new} < 0$ to satisfy the $C = 0$ constraint, which would effectively be an attractive force, which we don't want.
        \item Update the velocities for both bodies in collision $k$ immediately: $V = V + \Delta t M^{-1}J_k^T \Delta \lambda_k = V + \Delta V$ where $\Delta V$ is\\
        Circle-circle: $\Delta V = \Delta t \begin{pmatrix}-\vec{n}m_i^{-1}\\ \vec{n}m_j^{-1}\end{pmatrix}\Delta \lambda_k$\\
        Bounds-circle: $\Delta V = \Delta t \begin{pmatrix}0\\ - \vec{n}m_j^{-1}\end{pmatrix}\Delta \lambda_k$\\
